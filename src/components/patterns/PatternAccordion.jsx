...
export default function PatternAccordion({ pattern, problems, onAddProblem, onUpdateProblem, onDeleteProblem }) {
  ...
  return (
    <div className="border border-zinc-800 rounded-xl overflow-hidden bg-zinc-900/50 backdrop-blur-sm transition-all hover:border-zinc-700">
      <button
        onClick={() => setIsOpen(!isOpen)}
        className="w-full px-6 py-5 flex items-center justify-between text-left hover:bg-zinc-800/50 transition-all group"
      >
        <div className="flex items-center gap-4 flex-1">
          <div className={`w-10 h-10 rounded-lg bg-gradient-to-br from-blue-500 to-cyan-500 flex items-center justify-center transform transition-transform ${isOpen ? "rotate-90" : ""}`}>
